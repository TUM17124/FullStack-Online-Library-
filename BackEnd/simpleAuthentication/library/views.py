...
class RegisterView(APIView):
    ...
    try:
        with transaction.atomic():
            ...
            # EMAIL SENDING – DOUBLE-PROTECTED AGAINST CRASH
            email_sent = False
            email_error = None
            try:
                from django.core.mail import send_mail
...
Library Team
            ...
            return Response({
                'message': 'User registered successfully. Please check your email for the verification code.' if email_sent else 'User registered successfully (email sending failed - code shown for testing).',
                'email_status': 'sent' if email_sent else 'failed',
                'email_error': email_error if email_error else None,  # temporary
        ...
        print(f"Registration crashed: {str(e)}")  # outer catch-all
